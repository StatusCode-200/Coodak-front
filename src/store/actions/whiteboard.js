...
export const getWhiteboradAction = ({ challengeId, userId, token }) => (dispatch) => {
  dispatch(fetchWhiteboard());
  axios.get(`${API}/users/${userId}/challenges/${challengeId}/whiteboard`, { headers: { "Content-Type": "application/json" , Authorization: `Bearer ${token}` }})
    .then(({ data }) => {
...
export const postWhiteboardAction = ({ whiteboard, challengeId, userId, token }) => (dispatch) => {
...
const putWhiteboardStart = () => ({
  ...
  export const putWhiteboardAction = ({ whiteboard, challengeId, userId, token }) => (dispatch) => {///take the whiteboard from the state.whiteboard of WhiteBoardContainer.js  ///////check if the method override can work, otherwise use put and update server
